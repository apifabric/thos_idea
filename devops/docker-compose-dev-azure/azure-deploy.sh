...
resourcename="thosidea"  # lower case, only
resourcegroup="thosidea_rg"
...
# git clone https://github.com/apilogicserver/thosidea.git
# cd thosidea
...
az group create --name thosidea_rg --location "westus"
az appservice plan create --name myAppServicePlan --resource-group thosidea_rg --sku S1 --is-linux
az container create --resource-group thosidea_rg --name thosidea --image apilogicserver/thosidea:latest --dns-name-label thosidea --ports 5656 --environment-variables 'VERBOSE'='True'  'APILOGICPROJECT_CLIENT_URI'='//thosidea.westus.azurecontainer.io:5656'
